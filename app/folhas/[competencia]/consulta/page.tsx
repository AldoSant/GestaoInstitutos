...
const consultaSomenteLeitura = true;
...
export default async function FolhaDetalhePage({
  ...
  return (
      <AppShell
        title={`Consulta ${competencia(folha.competencia)}`}
      ...
      >
        <Link href={`/folhas/${folha.id}`} className="back-link"><ArrowLeft size={16} /> Voltar para a jornada</Link>
        {erro && (
          ...
          <div className="guided-actions">
            {!calculada ? (
              ...
              <>
                ...
                <Link className="button primary" href={`/folhas/${folha.id}`}>
                  Abrir jornada
                </Link>
              </>
            ) : !fechada ? (
        ...
        {calculada && conflitosDestaFolha.length > 0 && rateiosHomologados.length === 0 && (
          <section className="alert-box warning" id="consolidacao">
            ...
            <Link className="button primary" href={`/folhas/${folha.id}`}>
              Abrir jornada para consolidar
            </Link>
          </section>
        )}

        {folha.status === "RASCUNHO" && !consultaSomenteLeitura && (
        ...
        {["RASCUNHO", "ABERTA"].includes(folha.status) && !consultaSomenteLeitura && (
        ...
        {folha.status === "ABERTA" && !consultaSomenteLeitura && (
          <>
        ...
        {folha.status === "FECHADA" && !consultaSomenteLeitura && (
          <section className="panel">
            <div className="panel-header">
        ...
        <section className="panel" id="conferencia-arquivo">
          ...
          {folha.hash_resultado && !consultaSomenteLeitura ? (
            <form
            ...
            <p className="empty-state">
              Esta é uma consulta somente leitura. Para registrar uma nova
              referência, abra a jornada do processamento.
            </p>
          )}
